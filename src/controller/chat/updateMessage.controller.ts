  ...
  try {
    ...
    if (!message) {
      return res.status(404).json({
        ...
        statusCode: 404,
        errMsgs: {
          otherErr: { isErr: true, msg: "Message not found." },
        },
      });
    }
    ...
    const isOwner = (user.id === message.member.userId || !(member?.role === MemberRole.ADMIN || member?.role === MemberRole.MODERATOR))
    ...
    if (!isOwner) {
      return res.status(403).json({
        ...
        statusCode: 403,
        errMsgs: {
          otherErr: { isErr: true, msg: "You do not have permission to update this message." },
        },
      });
    }
